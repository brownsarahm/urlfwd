...
from .genpage import files_from_dict,create_landing,qrs_from_dict
...
def generate_qrs(source_yaml,retain,out_path,verbose,config_yaml,
                   landing, index):
    '''
    from a yaml source file create a set of toy files with file names as the keys
    and the values as the content of each file
    ...
    overwrite = not(retain)
    # call creator
    log = qrs_from_dict(files_to_create,overwrite,out_path,
                          logging=verbose,config_in=config)
